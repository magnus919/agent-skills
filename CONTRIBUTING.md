...
Paired-eval CI has a five-skill resource cap. When a change touches more than five skills with eval manifests, it fails selection rather than silently evaluating a subset. Split the PR or propose an explicit cap change. Read the selection summary before interpreting a green Jev audit: its percentage covers the prose assertions in comparison reports actually generated, not all changed skills or all catalog evals. If model evaluation did not run or a report is missing, say so; do not claim complete Jev coverage.
